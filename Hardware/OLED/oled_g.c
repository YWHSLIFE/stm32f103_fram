...
{
	OLED_SendBuff((uint8_t *)oled_buff);
}
	...
	switch (size)
	{
		case 16://16号字8x16
			for(uint8_t i=0;i<8;i++)
				{
					OLED_WinWriteByte(win,x,y,FF8X16[c*2][i]);
					OLED_WinWriteByte(win,x,y+8,FF8X16[c*2+1][i]);
		...
		case 24://24号字12*24
			for(uint8_t i=0;i<12;i++)
				{
					OLED_WinWriteByte(win,x,y,FF12X24[c*3][i]);
					OLED_WinWriteByte(win,x,y+8,FF12X24[c*3+1][i]);
					OLED_WinWriteByte(win,x,y+16,FF12X24[c*3+2][i]);
					x++;if(x > win->w)break; //已经超出边框没必要再写了
				}
			break;        
		case 8://12号字6*8
			for(uint8_t i=0;i<6;i++)
				{
					OLED_WinWriteByte(win,x,y,FF6x8[c][i]);
					x++;if(x > win->w)break; //已经超出边框没必要再写了
				}
			break;
		default:
